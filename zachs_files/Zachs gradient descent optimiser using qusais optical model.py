...
# -----------------------------
state_true = torch.zeros((image_size, image_size))

# Create a blob in the centre
cx, cy = image_size // 2, image_size // 2
for i in range(image_size):
    for j in range(image_size):
        state_true[i, j] = torch.exp(torch.tensor(
    -((i - cx)**2 + (j - cy)**2) / 20
))
...
for it in range(n_iters):
    ...
    for t in range(n_steps - 1):
        C_next_pred = advect(torch.sigmoid(C[t]))
        dyn_loss += torch.norm(torch.sigmoid(C[t+1]) - C_next_pred)**2
...
for t in range(n_steps):
    ...
    plt.title(f"Recovered Frame t={t}")
    plt.colorbar()
    plt.pause(1)
